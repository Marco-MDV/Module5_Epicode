...
export default function MyAlert() {
    ...
    const hendlerShow = () => {setShow(!show)}
    ...
    return (
        <>
            {show && (
                <Alert
                    variant={(selectTheme ? 'info' : 'dark')}
                    className={mainStyle.widthAlert + ' d-flex justify-content-around align-items-center myAlert '}
                >
                    <div className='w-100'>
                        <div className='position-relative w-100 d-flex justify-content-center align-content-center'>
                            ...
                            <button className={mainStyle.closeAllert + ' position-absolute top-0 end-0'} onClick={hendlerShow}>X</button>
                        </div>
                        <hr />
